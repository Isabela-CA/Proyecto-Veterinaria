...
public class ProcedimientoQuirurgicoDAO implements IProcedimientoQuirurgicoDAO {
    ...
    public List<ProcedimientoQuirurgico> listarProcedimientosQuirurgicos() {
        ...
        String sql = "SELECT * FROM procedimientos_quirurgicos ORDER BY fecha_procedimiento DESC, id DESC";
        ...
        try (Connection con = DatabaseConfig.getConexion();
            ...
            while (rs.next()) {
                var procedimiento = mapResultSetToProcedimiento(rs);
                procedimientos.add(procedimiento);
            }
        } catch (SQLException e) {
            System.out.println("Error al listar los procedimientos quirúrgicos: " + e.getMessage());
            e.printStackTrace();
        }
    ...
    public boolean buscarProcedimientoQuirurgico(ProcedimientoQuirurgico procedimiento) {
        ...
        var sql = "SELECT * FROM procedimientos_quirurgicos WHERE id = ?";
        try {
            ...
            System.out.println("Error al buscar el procedimiento quirúrgico por id: " + e.getMessage());
        } finally {
            try {
                con.close();
            } catch (Exception e) {
    ...
    public boolean agregarProcedimientoQuirurgico(ProcedimientoQuirurgico procedimiento) {
        String sql = "INSERT INTO procedimientos_quirurgicos(mascota_id, veterinario_id, cita_id, " +
        ...
        try (Connection con = DatabaseConfig.getConexion();
            ...
            System.out.println("Error al agregar el procedimiento quirúrgico: " + e.getMessage());
            e.printStackTrace();
            return false;
        }
    }
    ...
    public boolean modificarProcedimientoQuirurgico(ProcedimientoQuirurgico procedimiento) {
        String sql = "UPDATE procedimientos_quirurgicos SET mascota_id = ?, veterinario_id = ?, cita_id = ?, " +
                "fecha_procedimiento = ?, tipo_procedimiento = ?, diagnostico = ?, descripcion_procedimiento = ?, " +
                "anestesia_utilizada = ?, medicacion_prescrita = ?, cuidados_recomendados = ?, resultado = ? " +
                "WHERE id = ?";
        ...
        try (Connection con = DatabaseConfig.getConexion();
            ...
            System.out.println("Error al modificar el procedimiento quirúrgico: " + e.getMessage());
            e.printStackTrace();
            return false;
        }
    }
    ...
    public boolean eliminarProcedimientoQuirurgico(ProcedimientoQuirurgico procedimiento) {
        String sql = "DELETE FROM procedimientos_quirurgicos WHERE id = ?";
        ...
        try (Connection con = DatabaseConfig.getConexion();
            ...
            System.out.println("Error al eliminar el procedimiento quirúrgico: " + e.getMessage());
            e.printStackTrace();
            return false;
        }
    }
    ...
    public List<ProcedimientoQuirurgico> listarProcedimientosPorMascota(int mascotaId) {
        List<ProcedimientoQuirurgico> procedimientos = new ArrayList<>();
        String sql = "SELECT * FROM procedimientos_quirurgicos WHERE mascota_id = ? ORDER BY fecha_procedimiento DESC";
    ...
    public List<ProcedimientoQuirurgico> listarProcedimientosPorVeterinario(int veterinarioId) {
        List<ProcedimientoQuirurgico> procedimientos = new ArrayList<>();
        String sql = "SELECT * FROM procedimientos_quirurgicos WHERE veterinario_id = ? ORDER BY fecha_procedimiento DESC";
    ...
    public static void main(String[] args) {
        ProcedimientoQuirurgicoDAO procedimientoDao = new ProcedimientoQuirurgicoDAO();

        System.out.println("*** Listar PROCEDIMIENTOS QUIRÚRGICOS ***");
        var procedimientos = procedimientoDao.listarProcedimientosQuirurgicos();
        procedimientos.forEach(System.out::println);

        // Ejemplo: Listar procedimientos por mascota
        System.out.println("\n*** Listar PROCEDIMIENTOS por Mascota (ID: 1) ***");
        var procedimientosPorMascota = procedimientoDao.listarProcedimientosPorMascota(1);
        procedimientosPorMascota.forEach(System.out::println);
    }
}
